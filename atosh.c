...
/*   Updated: 2020/02/16 11:27:45 by pcharlot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
long long	atosh(char *s, int *i) ///вытаскивает из строки число, если число не входит в формат INTa - выдает ошибку
{
	...
	if (s[*i] == '-' || s[*i] == '+')
	{
		if(!is_digit09(s[*i + 1]))
		{
			write(2, "Error: знак + или - находится введен отдельно от цифр.", 90); ///Испавить воскл знаки - вариант когда кроме символов имеется хотя бы одно число, тогда именно здесь отлавливается ошибка
			exit (0);
		}
		if (s[*i] == '-')
			sign = -1;
